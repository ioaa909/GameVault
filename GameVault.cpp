...
void Save() {
    ...
    j+="\n]\n"; std::ofstream f(W2U(SaveP())); if(f.is_open()){f<<j;f.close();}
}
void Load() {
    std::ifstream f(W2U(SaveP())); if(!f.is_open()) return;
...
void AddGames() {
    ...
    ofn.lpstrFile=f; ofn.nMaxFile=32768;
    ofn.Flags=OFN_EXPLORER|OFN_ALLOWMULTISELECT|OFN_FILEMUSTEXIST;
    ofn.lpstrTitle=L"Select game executables";
    if(!GetOpenFileName(&ofn)) return;
    wchar_t* p=f; std::wstring dir=p; p+=dir.length()+1;
    bool added=false;
    if(*p==0) {
        for(auto& g:g_games) if(_wcsicmp(g.filePath.c_str(),dir.c_str())==0) return;
        GameEntry ge; ge.filePath=dir; ge.name=GetGN(dir); ge.icon=GetIcon(dir);
        g_games.push_back(ge); added=true;
    } else while(*p) {
        std::wstring path=dir+L"\\"+p; bool dup=false;
        for(auto& g:g_games) if(_wcsicmp(g.filePath.c_str(),path.c_str())==0){dup=true;break;}
        if(!dup){GameEntry ge;ge.filePath=path;ge.name=GetGN(path);ge.icon=GetIcon(path);g_games.push_back(ge);added=true;}
        p+=wcslen(p)+1;
    }
...
void RegUninst() {
    HKEY k;
    if(RegOpenKeyEx(HKEY_CURRENT_USER,L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\GameVault",0,KEY_READ,&k)==ERROR_SUCCESS){RegCloseKey(k);return;}
...
void RunUninst() {
    ...
    std::ofstream f(W2U(bp)); if(f.is_open()){std::string u=W2U(bat);f.write(u.data(),u.size());f.close();}
